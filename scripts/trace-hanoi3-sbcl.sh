...
TRACE_ROOT="$REPO_ROOT/analysis/hanoi3-traces"
...
EXPAND_BOUND=${EXPAND_BOUND:-5000}
GENERATE_BOUND=${GENERATE_BOUND:-20000}
OPEN_BOUND=${OPEN_BOUND:-20000}
CPU_SEC_LIMIT=${CPU_SEC_LIMIT:-30}
OPEN_SNAPSHOT_LIMIT=${OPEN_SNAPSHOT_LIMIT:-200}

timestamp=$(date +"%Y%m%d-%H%M%S")
case_slug="hanoi3-${PLANNER_MODE}-${HIERARCHY}-mp-${MP_MODE}-lw-${LEFT_WEDGE_MODE}-drp-${DRP_MODE}-${timestamp}"
