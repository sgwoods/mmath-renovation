...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/abtweak-hanoi3.XXXXXX")
...
run_tweak_case() {
  log_file="$TMP_DIR/hanoi3-tweak.log"
  eval_form='(progn
    (load "init-sbcl.lisp")
...
run_abtweak_case() {
  ...
  eval_form='(progn
    ...
    (let ((result (plan initial goal
                        ...
                        :open-bound '"$OPEN_BOUND"'
                        :cpu-sec-limit '"$CPU_SEC_LIMIT"')))
      (format t "CASE: hanoi3-'"$hierarchy"'-mp-'"$mp_mode"'-lw-'"$left_wedge_mode"'~%")
...
This report compares the historically relevant hanoi-3 hierarchy options in the current SBCL working tree.
...
- the permutation-style families ismb, imbs, ibsm, and isbm use the historical hanoi left-wedge list (0 1 3 7).
