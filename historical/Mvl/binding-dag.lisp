...
;; Many of these functions work by creating expressions to match and
;; then getting rid of them.  To make this a bit faster, we declare
;; these expressions to be of dynamic extent.

#-ANSI (proclaim '(declaration dynamic-extent))

;; The equality check is easy; the binding lists have to be of equal
;; length with one being less than the other.  This works because
;; ((? . a)) is *not* more specific than ((? . ?1)) -- consider the
;; result of applying both bindings to (x ? ?1).  (x a ?1) is not an
;; instance of (x ?1 ?1).
...
;; and to plug one binding list into another, we call append-binding-lists
;; but then discard any bindings from the plugger.

(defun binding-plug (b1 b2)
  (nset-difference (append-binding-lists b1 b2) b2 :key #'car))
...
(defun dot-binding (bl1 bl2)
  (cond ((null bl1) (list bl2))
	((null bl2) (list bl1))
	(t (let (exp1 exp2)
	     (declare (dynamic-extent exp1 exp2))
	     (dolist (item bl1)
...
(defparameter binding-dag
	      (make-dag :root nil :eq #'equal-binding :leq #'binding-le
			...
			:vars #'vars-in :plug #'binding-plug
...
(defun all-fn (var dag-list)
  (quantifier-fn var dag-list #'bilattice-and))

(defun exists-fn (var dag-list)
  (quantifier-fn var dag-list #'bilattice-or))

(defun quantifier-fn (var dag-list fn)
  (dag-change-dag dag-list
		  (if (listp var) #'(lambda (bdgs) (remove-bdgs var bdgs))
		      #'(lambda (bdgs) (remove-bdg var bdgs)))
		  :bilattice-modify fn))

;; here is where we actually make the modal operators.  Actually, since
;; the modal operators are standard ones, they already exist and all we
;; have to do is to change the associated functions to the above.

(defun create-quantifier-modalities ()
  (setf (modal-operator-function (bdg-modal-op 'forall)) #'all-fn
	(modal-operator-function (bdg-modal-op 'exists)) #'exists-fn))
