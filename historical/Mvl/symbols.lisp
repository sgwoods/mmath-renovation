...
;; types in.   They are not interned, and have the value
;; 'database-variable.
...
;; new uninterned variables are created that have the value
;; 'database-variable and have print names identical to those of the
;; original variables (so that the user can figure out what's going on).
;; Of course, when these variables are actually displayed, they will
;; frequently appear as #:?var since they are uninterned.
;;
  ...
  (if (listp p) 
    ...
    (let ((var (vartype p)))
      (cond ((null var) p)
	    ...
	    (t (setq var
		 (concsym nil var 
			  (subseq (symbol-name p) (if (eq var '?) 1 2))))
	       (push (cons p var) bdglist)
	       (set var 'database-variable)
	       var)))))

(defconstant *proposition-prefix* "P")
...
(defun new-meta-symbol (x &key (uniquify t)
			&aux (m (maksym *proposition-prefix*)))
  (if uniquify
...
;; Maksym makes a new symbol of the type given by string.
;; *maksym-alist* is an alist that holds the current count for each type
;; of symbol.

(defvar *maksym-alist* nil)

(defun maksym (string &aux (count (assoc string *maksym-alist* :test #'equal)))
  (unless count
    (push (setq count (cons string 0)) *maksym-alist*))
  (incf (cdr count))
  (concsym t string (cdr count)))
...
(defun concsym (intern? str1 str2)
  (setq str1 (format nil "~a~a" str1 str2))
  (if intern? (intern str1) (make-symbol str1)))
