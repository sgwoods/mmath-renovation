...
;;  1.  If they are equal (presumably both NIL), succeed.
...
(defun check-order (bdg blist)
  (if (or (database-variable (car bdg)) (assoc (cdr bdg) blist))
      bdg
    (if (varp? (car bdg))
	(if (and (symbolp (cdr bdg)) (database-variable (cdr bdg)))
	    (cons (cdr bdg) (car bdg))
	  bdg)
      (if (and (symbolp (second bdg)) (database-variable (second bdg)))
...
;; the * variable is bound, and the binding otherwise), and one for
...
(defun subst-bdg-special-case (bdg term &aux (var (car bdg)) found1 found2)
...
(defun subst-var (var &aux (bdg (assoc var finished-bdgs)) cell)
  ...
  (cond (bdg (cond ((eq (cdr bdg) '|BADLOOPCHECK|)
		    (throw 'no-unify 'fail))
		   (t (cdr bdg))))
...
(defun subst-term (term)
  (setq term (copy-list term))
  (do ((list term)) ((null list) term)
    (case (vartype (car list))
      (? (rplaca list (subst-var (car list)))
	 (pop list))
      (?* (let ((val (subst-var (car list))))
	    (cond ((null val)
		   (setq term (delete (pop list) term)))
		  ((atom val)			; no bdg for the var
		   (pop list))
		  (t (let ((tail (cdr list)))
		       (rplaca list (car val))
		       (rplacd list (append (cdr val) tail))
		       (setq list tail))))))
      (t (if (listp (car list)) (rplaca list (subst-term (car list))))
	 (pop list)))))
