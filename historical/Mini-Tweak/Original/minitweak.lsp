From shavlik@cs.wisc.edu  Tue Feb 13 14:20:40 1990
Received: from watsol.waterloo.edu by watdragon.waterloo.edu with SMTP
	id <AA21964>; Tue, 13 Feb 90 14:20:40 EST
Received: from steves.cs.wisc.edu by watsol.waterloo.edu with SMTP
	id <AA13423>; Tue, 13 Feb 90 14:20:15 EST
Date: Tue, 13 Feb 90 13:19:57 -0600
From: shavlik@cs.wisc.edu (Jude Shavlik)
Message-Id: <9002131919.AA21133@steves.cs.wisc.edu>
Received: by steves.cs.wisc.edu; Tue, 13 Feb 90 13:19:57 -0600
To: sgwoods@watsol
Subject: TWEAK.LSP
Status: R
...
;;;; Here's a good way to print code -> enscript -2rG  -fCourier7 <file>
...
;;;;   ************************* REQUIREMENTS *************************
;;;;
;;;;   Your assignment is to complete this program.
;;;;   This requires you finish the functions sketched in the section
;;;;   labeled "Secondary Functions."  You may write other functions if
;;;;   you wish.  (I only used one in my solution, but writing a lot of
;;;;   small functions is definitely in the spirit of Lisp.)  Place any
;;;;   additional functions you write in the section labeled "Tertiary
;;;;   Functions."  No other functions should require alteration.
;;;;   However, if you do find a "bug", please mail me at shavlik@cs.wisc.edu
;;;;   - I'll use the newsgroup cs.cs731 to post "bug" fixes and will keep
;;;;   an up-to-date version of the program on the HP Bobcats.
;;;;   (If someone writes a fancier function for printing out the final
;;;;   non-linear plan, let me see it and I may install it for everyone's use.)

;;;;   Demonstrate your solution works by running it on the domains
;;;;   defined in "~cs731/SAMPLE-TWEAK.DATA" and "~cs731/LENS-MAKER.DATA"
...
;;;;   Also devise a semi-realistic domain of your own, document it, and
;;;;   run your solution on it.  Draw (by hand) the partially-ordered plans that
;;;;   result for all THREE domains.   The program runs 'non-deterministically', so
;;;;   there isn't any one right answer; show the first complete solution you get
;;;;   (i.e., one that does not require backtracking to achieve the goal state).

;;;;   A compiled version of my solution is in "/home/inst/0/cs731/tweak-jws-soln.o"
;;;;   You may use this to get an understanding of what the program is to do.
;;;;   It may also prove useful for debugging your personal domain.  The flag
;;;;   *TRACE-TWEAK* controls how much information is provided by MINI-TWEAK.
;;;;   Don't waste disk space by copying my solution, just load it from the cs731
;;;;   directory.  (Note:  Our Common Lisp (KCL) cannot handle the "~" notation for
...
;;;;   Besides the hand-sketched non-linear plans, turn in a 'dribble' file
;;;;   of your program in action.  Also turn in a printout of your 'personal' domain.
;;;;   Finally, turn in a printout of the code you wrote.  ONLY TURN IN THE CODE YOU
;;;;   WROTE - NOT THE COMPLETE PROGRAM.

;;;;   Your solution will be "auto-graded" on a novel domain. Make sure your solution
;;;;   is in a file called MINI-TWEAK.LSP (uppercase is important).  This file MUST be
;;;;   in your "top-level" (i.e., login) directory and it should contain the COMPLETE
;;;;   RUNNABLE PROGRAM - not just the code you write.  Read the CS540 handout at MACC
;;;;   on the AI Programming Environment to see how you can use the auto-grader 
;;;;   yourself.  Please test that everything works w/o problem - this will greatly
;;;;   ease the grading burden.  Thanks.

;;;;   ************************ Representing Plan Steps ************************
;;;;   TWEAK works by inserting steps (operator applications) into a plan to achieve
;;;;   goals, partially ordering them to avoid conflicts.  In MINI-TWEAK, assume a plan
;;;;   step is presented by two states - the time immediately before the plan step and
;;;;   the point in time immediately after it. (Except for the initial and goal states,
;;;;   all states arise from operator applications.)  Say MINI-TWEAK needs to achieve
;;;;   predicate P in state S and chooses to use operator O.  You should generate two
...
(defun run-mini-tweak (&optional 
		       ...
		       (operator-file "/home/inst/0/cs731/SAMPLE-TWEAK.DATA"))
...
;;; so that MINI-TWEAK reports its progress.  Also feel free to define additional 
;;; functions - add them in the TERTIARY FUNCTIONS group. 
;;; (Mail bug reports to shavlik@steves or post them in the newsgroup cs.cs731
;;;  - the unix program Pnews allows to post a message, rn supports reading messages.)
