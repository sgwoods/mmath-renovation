...
(defun find-crit (pname &key (operator nil))
   "abtweak/ab-successors.lsp
    return the criticality level of a precondition
    optionally consider the operator in the calculation"
   (declare 
      (type list pname))
    ...
    (if (null operator)
        ...
        (let* (
               (op-name (first (operator-name operator))) 
               (p-name  (car (strip-negate pname))) )
         (if (eq p-name 'clear)
             (if (eq op-name 'move-big)
                 *move-big-clear-criticality*
                 (if (eq op-name 'move-medium)
                     *move-medium-clear-criticality*
                     (if (eq op-name 'move-small)
                         *move-small-clear-criticality*
                         *default-clear-criticality*)))) ))
   )
