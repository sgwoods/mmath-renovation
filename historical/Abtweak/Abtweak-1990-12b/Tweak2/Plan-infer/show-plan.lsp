...
; modified by qiang yang, nov  1990
...
(defun show (plan) 
  "/tweak/plan-infer/show-plan.lsp
   ...
   (show-ops plan :show-cr t))

; include file from qiang : tw-interface.lsp
...
(defun sort-orders (op-list orders)
  "returns a slightly better ordering.
   orders starting from op-list are first returned.
   then those that are connected by those in op-list.  recursive"
  (let ((first-orders
	 (remove-if-not
	  #'(lambda (order)
	      (member (first order) 
		      op-list))
	  orders)))
    (cond ((null orders) nil)
	  (t (append first-orders
		   (sort-orders 
		    (mapcar #'(lambda (order)
				(second order))
			    first-orders)
...
(defun shownc (plan)
  "/tweak/plan-infer/show-plan.lsp
   ...
   (declare 
       (type plan plan))
   (princ "non codesignations: ")(terpri)(princ (plan-nc  plan)) (terpri)
   (terpri)
 )
