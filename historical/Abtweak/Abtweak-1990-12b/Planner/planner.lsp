...
(defvar *k-branching*      0    "record branching values")
...
(defvar *determine-mode*   nil  "deter. u and p mode = random / tree / first")
...
(defun planner (initial goal &key 
  ...
  (declare 
        ...
        (type atom determine-mode)           ; random / tree / weak
        (type boolean backwards-mode)        ; t / nil
        (type boolean k-learn-mode)          ; t / nil
        (type boolean debug-mode)            ; t / nil
        (type integer search-type)           ; 0 - 6
        (type atom    heuristic)             ; id code, default = d
        (type integer dfs-depth-limit)       ; n
        (type boolean verbose-p)             ; t / nil
        (type boolean print-opt-p)           ; t / nil
   )
  ...
  (setq *k-learn-mode*    k-learn-mode)
  (setq *debug*           debug-mode)
  (setq *search*          search-type)
  (setq *heuristic*       heuristic)
  (setq *dfs-limit*       dfs-depth-limit)
  (setq *verbose-p*       verbose-p)
  (setq *print-opt-p*     print-opt-p)
...
(defun tw-show-dom ()
  "planner/planner.lsp
   tweak - print out domain information"
          ...
          (princ "operators    : ") (terpri) (princ *operators*) (terpri)
...
(defun ab-print-stats ()
  "planner/planner.lsp
   print out run statistics, in abtweak domain"
    ...
    (princ "a: k learning mode       : ") (princ *k-learn-mode*)       (terpri)
...
(defun ab-show-dom ()
  "planner/planner.lsp
   print out domain information"
     (tw-show-dom)
     (princ "a: criticalities      : ") (princ *critical-list*)   (terpri) 
     (princ "a: criticality id     : ") (princ *critical-loaded*) (terpri) 
     (princ "a: eliminate td levels: ") (princ *top-down-levels*) (terpri) 
     (princ "a: critcal depth modes: ") (princ *k-list*)          (terpri)
     (terpri) )
